...
class NoAtt(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev):
        ...
        :param list enc_h_len: padded encoder hidden state lenght (B) 每一个序列不一样长， enc_h_len 表示每一个序列长度。 seq_len
        ...
        :rtype: Variable
        '''
        batch = len(enc_hs_pad) # enc_hs_pad (batch,seq_len,dim)
        ...
        if att_prev is None:
            ...
            # list every units is a variable the length is enc_hs_len[i] len(att_prev) = len(enc_hs_len) = b
            # if no bias, 0 0-pad goes 0
            att_prev = pad_list(att_prev, 0) # 把列表 补到一样长 b,t,dim
            self.c = torch.sum(self.enc_h * att_prev.view(batch, self.h_length, 1), dim=1)
            # 用公式把atten_weight 和encoder hidden 做加 a_11*h1+a_12*h2 ....
            # 对于一个decode size (b, enc_dim)
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=2.0):
        ...
        if self.pre_compute_enc_h is None:
            ...
            self.pre_compute_enc_h = torch.tanh(
                linear_tensor(self.mlp_enc, self.enc_h)) # mlp_enc (eprojs,att_dim), enc_h (B,t,D) return: (B,t,att_dim)

        if dec_z is None:
            dec_z = Variable(enc_hs_pad.data.new(batch, self.dunits).zero_())  # (batch dunits)
        else:
            # 当前dec size(batch,1,dim)
        ...
        c = torch.sum(self.enc_h * w.view(batch, self.h_length, 1), dim=1) # (b,enc_dim) context vector
        return c, w
...
class AttLoc(torch.nn.Module):
    ...
    def __init__(self, eprojs, dunits, att_dim, aconv_chans, aconv_filts, aact_fuc):
        ...
        # 经过conv 参数设置输出和输入形状在H,W一样，输出为（1,aconv_chans,H_input,W_input)
        self.loc_conv = torch.nn.Conv2d(
            1, aconv_chans, kernel_size = (1, 2 * aconv_filts + 1), padding=(0, aconv_filts), bias=False) # (input_c,output_channel,H,W)
        self.gvec = torch.nn.Linear(att_dim, 1)
...
class AttLoc2D(torch.nn.Module):
    ...
    def __init__(self, eprojs, dunits, att_dim, att_win, aconv_chans, aconv_filts):
        ...
        self.mlp_dec = torch.nn.Linear(dunits, att_dim, bias=False)
        self.mlp_att = torch.nn.Linear(aconv_chans, att_dim, bias=False)
        #  after conv output size (input_channel,aconv_chans,H_in-att_win+1,W_in)
        self.loc_conv = torch.nn.Conv2d(
