...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, etype, idim, elayers, eunits, eprojs, subsample, subsample_type, dropout, in_channel=1):
        ...
        if etype == 'blstm':
            self.enc1 = BLSTM(idim, elayers, eunits, eprojs, dropout) # euiits hidden_dim  eprojs h_dim for linear transform
...
class BLSTMP(torch.nn.Module):
    ...
    def forward(self, xpad, ilens):
        ...
        for layer in six.moves.range(self.elayers):
            ...
            ypad, ilens = pad_packed_sequence(ys, batch_first=True) #yoad (b,t,dim)
            sub = self.subsample[layer + 1]
            if sub > 1:
                #print(layer, sub, ypad.shape)
                if self.subsample_type == 'skip':
                    ypad = ypad[:, ::sub] #sub sample for the 1 dim. seq_LENGTH
                    ilens = [int(i + 1) // sub for i in ilens] # 计算新的 ilens 时间维度下采样
                elif self.subsample_type == 'maxpooling':
                    ypad = ypad.transpose(1, 2) #y_pad (b,dim,t)
                    ypad = F.max_pool1d(ypad, int(sub), stride=int(sub))
                    ypad = ypad.transpose(1, 2) # ypad (b,t,dim)
                    ilens = [int(i) // sub for i in ilens]
            # (sum _utt frame_utt) x dim
            projected = getattr(self, 'bt' + str(layer)
                                )(ypad.contiguous().view(-1, ypad.size(2))) # 线性变换，输入dim 2*c_dim
...
class BLSTM(torch.nn.Module):
    ...
    def forward(self, xpad, ilens):
        ...
        '''
        #logging.info(self.__class__.__name__ + ' input lengths: ' + str(ilens))
        xpack = pack_padded_sequence(xpad, ilens, batch_first=True) # xpad 形状 (b,t,dim) b batch t seq_length dim input dim ilens: 每个tensor 的长度，不加pad  输出.data （sum(ilen),dim)
        ys, (hy, cy) = self.nblstm(xpack)
        del hy, cy
        # ys: utt list of frame x cdim x 2 (2: means bidirectional)
        ypad, ilens = pad_packed_sequence(ys, batch_first=True) #return (b,t,dim)
        # (sum _utt frame_utt) x dim
