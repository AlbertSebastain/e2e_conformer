...
class EnhanceModel(ModelBase):
    def __init__(self, args):
        ...
        # idim input dim 应该是 batch, n_frame,257?
        self.opt = args
        idim = args.idim 
        odim = args.idim 
        self.enhance_type = args.enhance_type # e.g. BLSTM
    ...
    def forward(self, mix_inputs, mix_log_inputs, input_sizes, clean_inputs=None, cos_angles=None):
        ...
        for i, length in enumerate(ilens):
            ...
            if (mask[i].size(0) - length) > 0:
                #mask size (b,seq_len,odim)
                # dim 0 取出 length 到 mask[i].size-
                mask[i].narrow(0, length, mask[i].size(0) - length).fill_(1)
                # 变换以后，每个 mask[i] size (seq_len,odim) 前length 处 0 后面全部变成1
                # masked_fill for out[i] the first length items will be not changed the others are zero
        ...
        if clean_inputs is not None:
            ...
            loss = F.l1_loss(enhance_out, clean_inputs * cos_angles, reduction = 'sum')
            # ilens 是 seq_len
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if input_nc is None:
            input_nc = outer_nc
            # input_nc in_channel inner_nc output_channel
            #downconv (B,inner_nc,h,w)
        downconv = nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        downrelu = nn.LeakyReLU(0.2)

        # norm_layer 对于batchnorm2d inner_nc 指num_feature, C Of (N,C,H,W)
