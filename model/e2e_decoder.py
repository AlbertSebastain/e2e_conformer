...
class Decoder(torch.nn.Module):
    def __init__(self, eprojs, odim, dlayers, dunits, sos, eos, att, verbose=0,
        ...
        # embedding odim 词典长度，one hot 编码，成为dunits
        self.embed = torch.nn.Embedding(odim, dunits)
        self.decoder = torch.nn.ModuleList()
        self.fusion = fusion
        self.rnnlm = rnnlm
        # input_size dunits+eprojs
        # hidden_size = dunits
        # 输入shape dunits 指的是 embed 以后的dim eprojs 指的是 att_c 的h_dim
    ...
    def forward(self, hpad, hlen, ys, scheduled_sampling_rate):
        ...
        # 这里用lstmcell, 时间和层需要实现， 用两个循环，第一个控制时间seq, 第二层控制层
        for i in six.moves.range(olength):
            # 总的来说，decoder 结构是一共有dlayers层，每一层有1个rnn,一共有olength长度。
            # 首先对每个时间和每一层展开，如果是0个时间或者 random.random()> sampling rate
            # 当前的输入是真实标签，如果不满足，则输入是上一轮输出的最可能值，进行embed。 通过所有encoder 状态和前一个时间的hidden state
            # 叫做 z_list[0] 0 代表第0层rnn 不代表时间是0。
            # 还有前一步的att_w，也许没有用（dot atten add atten 没有用，为了接口统一，也要算arg) 计算 att, att_c是 content vector
            # 意思是已经加权得到的 shape (batch,enc_dim) att_w 是 权重，shape (b,seq_len) 有了这两个，在接下来对输入向量和att_c进行串联
            # shape (batch,dunits+eprojs) dunits 是经过embed的dim eprojes 是encoder的dim。
            # 这个大的向量作为输入进行rnn, 同时对每一层进行计算，最后得到当前时间的输出 （batch，dunits) 进行一个线性层，输出是 (batch,oodim)
            # odim 是字典大小，表示有多少词，可以表示为score, 为后面选取最高分准备。（流程结束）
            # 对于一个decoder状态，算att. att_c, (b,enc_dim), att_w (b,seq_len)
            # z_list[0] current decode hidden state (batch,dec_dim)
            # att_w (在输入参数 prev_att_w)
            att_c, att_w = self.att(hpad, hlen, z_list[0], att_w)
            if random.random() < scheduled_sampling_rate and i > 0:
                # topv yi 最有可能值， topi 位置 b,1 y_i 猜测是字母表的概率
                topv, topi = y_i.topk(1) # y_i shape （batch, odim） 输出的top_y shape （batch,1) 找出每一行最大的，意思是找出概率最大
                topi = topi.squeeze(1)
                ey_top = self.embed(topi)  # utt x zdim
                ey = torch.cat((ey_top, att_c), dim=1)  # utt x (zdim + hdim)              
            else:
                # pad_ys_in (b,seq_len,dim) the label with padding 取i个时间的内容
                topi = pad_ys_in[:, i]
                # 用ey 意义是真实label 放到解码的rnn？
                # eys 真实数据 embed 以后的向量
                ey = torch.cat((eys[:, i, :], att_c), dim=1)  # utt x (zdim + hdim)  在程序里 zdim 为 dunits hdim 为 eprojs
            z_list[0], c_list[0] = self.decoder[0](ey, (z_list[0], c_list[0]))
            # 一共dlayers 层， 每一层1个rnn
            for l in six.moves.range(1, self.dlayers):
            ...
            if self.fusion == 'deep_fusion' and self.rnnlm is not None:
                ...
                # z_list 是输出状态？ z_list[-1] 是最后一层的一个rnn 在一个 time 里的输出
                output_in = z_list[-1]
            y_i = self.output(output_in)  # output 线性层 （dunits,odim  odim 为字典大小，也就是有多少词
            y_all.append(y_i)                   
