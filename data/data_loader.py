...
class SequentialDataset(Dataset, FbankFeatLabelParser):
    def __init__(self, args, data_dir, dict_file, type_data, noise_level = 'clean',mct = False):
        ...
        with open(self.speech_scp, encoding='utf-8') as f: #打开speech_scp 存储feates
            spe_ids = f.readlines()  #每行返回一个列表
        self.spe_ids = [x.strip().split(' ') for x in spe_ids]  #按空格分出来strip去掉空格
        self.spe_size = len(self.spe_ids)  #音频文件个数
