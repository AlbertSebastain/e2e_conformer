...
asr_name="aishell_asr_conformer_model"
joint_name="conformer_joint_train"
...
if [ ${stage} -le 3 ]; then
    ...
    cp ${dataroot}/${train_set}/clean_feats.scp ${dataroot}/${train_set}/feats.scp
    cp ${dataroot}/${train_dev}/clean_feats.scp ${dataroot}/${train_dev}/feats.scp
    cp ${dataroot}/${test_set}/clean_feats.scp ${dataroot}/${test_set}/feats.scp
    batchsize=15
    epochs=30
...
if [ ${stage} -le 4 ]; then
    ...
    bash ./recog_conformer.sh "clean_recog" "${asr_name}" "test"
...
if [ ${stage} -le 6 ]; then
    ...
    bash ./joint_recog_conformer.sh "enhance_recog" ${enhance_name} ${asr_name} "test"
...
if [ ${stage} -le 7 ]; then
    ...
    cp ${exp_root}/${asr_name}/model.acc.best ${exp_root}/${joint_name}/model.conformer.best
    enhance_resume=model.enhance.best
    asr_resume=model.conformer.best
    config_file_joint_train=config/joint_train_config.yml
    python3 joint_train_conformer.py \
        --dataroot $dataroot \
        --name $joint_name \
        --dict_dir ${dictroot} \
        --config_file ${config_file_general} \
...
if [ ${stage} -eq 8 ]; then
    ...
    bash ./joint_recog_conformer.sh "joint_train_recog" ${enhance_name} ${joint_name} "test"
