...
def train():
    ...
    if (opt.e2e_resume != None) & (opt.enhance_resume != None):
        ...
        if os.path.isfile(e2e_model_path) & os.path.isfile(enhance_model_path):
            ...
            package = torch.load(e2e_model_path,map_location=lambda storage, loc:storage)
            step = int(package.get('iters', 0))   
    ...
    if opt.opt_type == 'noam':
        optimizer = NoamOpt(asr_model.adim, 1, 25000, optimizer,step)
    asr_model.cuda()
    print(asr_model)
    asr_model.train()
    #enhance_model.eval()
    ...
    if os.path.exists(fbank_cmvn_file):
            ...
            for i, (data) in enumerate(train_loader, start=0):
                ...
                fbank_cmvn = fbank_model.compute_cmvn(mix_inputs, input_sizes)
